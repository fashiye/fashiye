...
from app.核心.配置 import 配置对象                   # 应用全局配置
import logging                                        # 记录错误日志
...
async def 发起支付接口(
    ...
    try:
        ...
        支付信息 = await 支付服务对象.创建支付订单(
            数据库, 订单, 请求数据.联系方式,
            支付方式=请求数据.支付方式,
            重定向地址=请求数据.重定向地址 or f"{配置对象.前端基地址}/order-detail/{请求数据.订单ID}",
        )
...
async def 查询支付状态接口(
    ...
    # 验证当前用户与订单的关系（仅订单所有者、接单打手、管理员可查）
    ...
    # 调用库函数：判断是否有权限查询支付状态
    # 传入：当前用户ID、当前角色、订单的用户ID和打手ID
    # 作用：确保只有订单参与者或管理员才能查看支付明细
    # 传出：无返回值（无权时抛异常）
    if 当前角色 == "admin":
        pass  # 管理员可查所有
    elif 当前角色 == "user" and 订单.用户ID == 当前用户.id:
        pass
    elif 当前角色 == "handler" and 订单.打手ID == 当前用户.id:
        pass
    else:
        raise 业务逻辑错误("您无权查看此订单的支付状态")
    ...
    return {
        ...
        "data": [
            {
                "id": 记录.id,
                "amount": str(记录.支付金额) if 记录.支付金额 else None,
                "status": 记录.支付状态,
